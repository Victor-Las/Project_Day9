...
def number_handle (journalists)
  puts "1. Combien y'a-t-il de handle dans cette array?"
  puts "Il y a #{journalists.count()} handle dans cette array."
  puts  
end
...
def shortest_handle (journalists)
  ...
  puts "Le handle le plus court du array est: #{journalists.min {|a,b| a.length <=> b.length}}"
  puts
end

#Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère) ?
def five_characters_handles (journalists)
  puts "3. Combien y-a-t'il de handle contenant 5 caractères ?"
  puts "Il y a #{journalists.count {|i| i.length == 6}} handle contenant 5 characteres."
  puts
end

#Combien commencent par une majuscule (première lettre juste après le @) ?
def uppercase (journalists)
  puts "4. Combien de handle commencent par une majuscule ?"
  x = journalists.count {|i| i[1,1] =~ /[A-Z]/}
  puts "#{x}"
  puts
end

#Trie la liste de handle par ordre alphabetique. 
def sort_alpha (journalists)
  puts "5. Trie la liste de handle par ordre alphabetique."
  journalists_sorted_alpha = journalists.sort
  puts "#{journalists_sorted_alpha}"
  puts
end

#Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après).
def sort_size (journalists)
  puts "6. Trie la liste de handle par taille des handle."
  journalists_sorted_size = journalists.sort_by(&:length)
  puts "#{journalists_sorted_size}"
  puts
end

#Quelle est la position dans l'array de la personne @epenser ?
def where_is_epenser (journalists)
  puts "7. Quelle est la position dans l'array de la personne @epenser ?"
  epenser_position = journalists.index("@epenser")
  puts "@epenser est en #{epenser_position}eme position"
  puts
end
...
def repartition (journalists)
  ...
  i = 1
  while i < 33
    j = journalists.count{|k| k.length == i + 1}
    puts "Il y a #{j} journalistes dont le handle comporte #{i} caractères."
    i +=1
  end
  puts
end
...
def answer_questions
  ...
  while question_number != 0
    ...
    puts "9: Toutes les questions."
    puts "0: Quitter le programme"
    puts
    puts "Entrez le numéro de question souhaité:"
    print "> "
    question_number = gets.chomp.to_i
    puts
    case question_number
    when 9
      number_handle (journalists)
      shortest_handle (journalists)
      five_characters_handles (journalists)
      uppercase (journalists)
      sort_alpha (journalists)
      sort_size (journalists)
      where_is_epenser (journalists )
